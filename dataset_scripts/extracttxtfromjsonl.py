...
def main(jsonl_file, caption_field):
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            # Extract the 'caption_field' field
            caption_value = matching_object.get(caption_field)
            if not caption_value:
                print(f"No '{caption_field}' found for URL: {url}", file=sys.stderr)
                continue

            # Create the .txt file and write the data
            txt_file_path = Path(json_file_path).with_suffix('.txt')
            with open(txt_file_path, 'w', encoding='utf-8') as txt_file:
                txt_file.write(caption_value)
