...
( testdir currently is /data2/LAION/LAION_2B_square-womansolo )

create_t5cache_768_h5.py
~~~~~~~~~~~~~~~~~~~
...
def main():
    ...
    for start in tqdm(range(0, len(txt_files), bs), unit="batch"):
        ...
        for path, vec in zip(batch_files, emb):
            outpath = path.with_suffix(path.suffix + CACHE_POSTFIX)
            with h5py.File(outpath, "w") as h5f:
                h5f.create_dataset("emb", data=vec.to(torch.float32).numpy(), compression="gzip")
                # Can use "lzf" for 20%+ speed, but at 20% space penalty also
