...
# But not going to remove it just yet
"""
reconstruct_ema.py

Materialize a full, loadable EMA VAE from a direct-save EMA snapshot
(see train_ema.py). Snapshots are online EMAs of the trainable params,
saved fp32 as ema_t*_sr*.safetensors. This tool merges one such shadow
onto a raw checkpoint (for frozen params, buffers, and config) and
writes a standalone VAE dir.

Usage:

    reconstruct_ema.py --ema_dir <out>/ema \
...
def scan_snapshots(ema_dir: Path):
    """Return [(t, sigma_rel, path)] for every EMA snapshot in the dir."""
    snaps = []
    for f in sorted(ema_dir.glob("ema_t*_sr*.safetensors")):
        m = SNAP_RE.search(f.name)
        if m:
            snaps.append((int(m.group(1)), float(m.group(2)), f))
    if not snaps:
        raise SystemExit(f"No ema_t*_sr*.safetensors snapshots in {ema_dir}")
    return snaps


def pick_snapshot(ema_dir: Path, sigma_rel: float, step: int | None) -> Path:
    """Snapshot for sigma_rel at t==step, or newest t if step is None."""
    cands = [(t, f) for t, sr, f in scan_snapshots(ema_dir)
             if abs(sr - sigma_rel) < 1e-6]
    if not cands:
        raise SystemExit(f"No snapshots with sigma_rel={sigma_rel:.4f} in {ema_dir}")
    if step is None:
...
def main():
    ...
    ema_file = pick_snapshot(Path(args.ema_dir), args.sigma_rel, args.step)
